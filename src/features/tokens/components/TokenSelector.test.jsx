...
function marketToken(overrides = {}) {
    const chainId = Number(overrides.chainId ?? 56)
    const address = overrides.address ?? '0x0000000000000000000000000000000000000001'
    return {
        id: `${chainId}:${String(address).toLowerCase()}`,
        chainId,
        address,
        isNative: false,
        name: 'Market Token',
...
describe('TokenSelector wallet rows', () => {
    ...
    it('keeps unknown positive wallet tokens in one protected section when allowed', () => {
        ...
        const protectedSection = screen.getByText('Protected tokens (4)').closest('section')
        fireEvent.click(within(protectedSection).getByRole('button', { name: 'Show' }))
        ...
        expect(screen.getByText(/unverified or risky/i)).toBeTruthy()
        ...
        expect(screen.queryByText('Protected tokens (4)')).toBeNull()
        expect(screen.queryByText(hidden.name)).toBeNull()
        expect(screen.queryByText(unverified.name)).toBeNull()
    })
    ...
    it('never labels native BNB as a potential risk', () => {
        render(
            <TokenRow
                token={{
                ...native,
        ...
        const row = screen.getByText('BNB', { selector: 'strong' })
            .closest('.ps-token-row')
        expect(within(row).queryByText('Potential risk')).toBeNull()
    })
    ...
    it('hides zero-balance wrapped-native catalog rows from all-chain eth search while keeping native ETH', () => {
        renderSelector({
            chainId: 'all',
            search: 'eth',
            tokens: [
            ...
            walletTokens: [ethNative],
            currentToken: null,
        })

        expect(screen.getByText('Ether', { selector: 'strong' })).toBeTruthy()
        expect(screen.getByText('ETH', { selector: '.ps-token-symbol' })).toBeTruthy()
        expect(screen.queryByText('Wrapped Ether')).toBeNull()
        expect(screen.queryByText('WETH', { selector: '.ps-token-symbol' })).toBeNull()
    })

    it('hides BNB Chain wrapped-native catalog rows from bnb search while keeping native BNB', () => {
        renderSelector({
            chainId: 56,
            search: 'bnb',
            tokens: [
        ...
        expect(screen.getByText('BNB', { selector: 'strong' })).toBeTruthy()
        expect(screen.queryByText('Wrapped BNB')).toBeNull()
        expect(screen.queryByText('WBNB', { selector: '.ps-token-symbol' })).toBeNull()
    })

    it('keeps positive wallet-owned WETH under Your tokens once', () => {
        const weth = marketToken({
            chainId: 1,
            address: '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2',
            name: 'Wrapped Ether',
            symbol: 'WETH',
        ...
        const walletSection = screen.getByText('Your tokens').closest('section')
        expect(within(walletSection).getByText('Wrapped Ether')).toBeTruthy()
        expect(screen.getAllByText('Wrapped Ether')).toHaveLength(1)
    })

    it('allows exact wrapped-native contract address search', () => {
        renderSelector({
            chainId: 1,
            search: '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2',
            tokens: [
                marketToken({
                    chainId: 1,
                    address: '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2',
        ...
        expect(screen.getByText('Wrapped Ether')).toBeTruthy()
        expect(screen.getByText('WETH', { selector: '.ps-token-symbol' })).toBeTruthy()
    })

    it('keeps non-native wrapped assets visible', () => {
        renderSelector({
            chainId: 1,
            search: 'wrapped',
            tokens: [
        ...
        expect(screen.getByText('Wrapped Bitcoin')).toBeTruthy()
        expect(screen.getByText('WBTC', { selector: '.ps-token-symbol' })).toBeTruthy()
        expect(screen.getByText('Random Wrapped Example')).toBeTruthy()
    })
