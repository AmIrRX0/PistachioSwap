...
function InaccessibleWalletContent({ onRestore, onSensitiveChange, onStart, snapshot }) {
    ...
    const signingOnly = snapshot.signingPasskeyOnly === true &&
        typeof manager.activateReadOnlySession === 'function'
    ...
    async function continueReadOnly() {
        ...
        setError(null)
        try {
            await manager.activateReadOnlySession(vault.vaultId)
        } catch (nextError) {
            setError(nextError)
        } finally {
            setBusy(false)
...
function SavedWalletEntry({ onAnother, onChoose, onRestore, onSensitiveChange, onStart, snapshot }) {
    ...
    async function continueWithWallet() {
        ...
        try {
            if (signingOnly) {
                await manager.activateReadOnlySession(snapshot.vault.vaultId)
            } else {
                await manager.unlock()
            }
        } catch (nextError) {
    ...
    async function openSecurityTools() {
        if (busy || securityBusy) return
    ...
    return (
        <div className="pistachio-wallet-stack">
            ...
            <EntryButton tone="primary" icon={signingOnly ? WalletCards : KeyRound} description={signingOnly ? 'No passkey needed for view-only access' : 'Unlock with your passkey'} disabled={busy || securityBusy} onClick={continueWithWallet}>{busy ? 'Opening wallet…' : signingOnly ? 'Continue with this wallet' : 'Use previous wallet'}</EntryButton>
            <div className="pistachio-wallet-entry-actions pistachio-wallet-secondary-actions">
                {signingOnly && <EntryButton icon={KeyRound} description="Passkeys, backups, and recovery tools" disabled={busy || securityBusy} onClick={openSecurityTools}>{securityBusy ? 'Waiting for passkey…' : 'Wallet security & backup'}</EntryButton>}
...
function SavedWalletChooser({ onBack, onRestore, onSensitiveChange, onStart, snapshot }) {
    ...
    async function openVault(vaultId) {
        await runForVault(vaultId, async () => {
            ...
            if (signingOnly) await manager.activateReadOnlySession(vaultId)
            else await manager.unlock()
        })
    }
    ...
    return (
        <div className="pistachio-wallet-stack">
            <BackButton onClick={onBack} />
            <ScreenIntro title="Saved Pistachio Wallets">{signingOnly ? 'Choose a wallet to view. Passkey confirmation happens only when a sensitive action is requested.' : 'Choose which encrypted wallet to unlock.'}</ScreenIntro>
                    ...
                    <article className="pistachio-saved-wallet" key={vault.vaultId}>
                        ...
                        {renameTarget !== vault.vaultId && (
                            <div className="pistachio-saved-wallet-actions">
                                <button className="pistachio-wallet-primary" type="button" disabled={Boolean(busyVaultId)} onClick={() => openVault(vault.vaultId)}>{signingOnly ? <WalletCards aria-hidden="true" /> : <KeyRound aria-hidden="true" />} {busyVaultId === vault.vaultId ? 'Opening…' : signingOnly ? 'Continue' : 'Unlock'}</button>
