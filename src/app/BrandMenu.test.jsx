    ...
    describe('desktop', () => {
        ...
        it('opens when hovering the brand while keeping the logo as a landing link', () => {
            ...
            expect(screen.getByRole('link', { name: /Pistachio Wallet/ }).getAttribute('href'))
                .toBe('/landing/wallet/')
            expect(screen.getByRole('link', { name: /How Pistachio Swap works/ }).getAttribute('href'))
                .toBe('/landing/how-it-works/')
            ...
            expect(screen.queryByText('Smart routing')).toBeNull()
            expect(screen.queryByText('Multi-chain')).toBeNull()
    ...
    describe('mobile', () => {
        ...
        it('keeps the logo as a landing link and opens a bottom sheet from the hamburger', () => {
            ...
            expect(screen.getByRole('button', { name: 'Products' }).getAttribute('aria-expanded'))
                .toBe('false')
            expect(screen.queryByRole('button', { name: 'Protocol' })).toBeNull()
            expect(screen.queryByRole('button', { name: 'Company' })).toBeNull()
            expect(screen.getAllByRole('link', { name: /^Trade$/ })).toHaveLength(2)
            expect(screen.getByRole('link', { name: /^Gas Assist$/ })).toBeTruthy()
