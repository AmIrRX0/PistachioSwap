...
function usesRiskPresentation(token) {
    return token?.visibility === 'hidden' ||
        token?.visibility === 'unverified' ||
        token?.possibleSpam === true ||
        ['hidden', 'blocked'].includes(String(token?.classificationTier)) ||
...
/**
 * Renders one selectable token row, including identity, security, owned balance, and context callbacks.
 ...
 * @param {boolean} [props.showBalance=false] Shows the connected wallet's total USD value and token quantity.
...
export function TokenRow({ token, currentToken, oppositeToken, showBalance = false, onSelect, onContextMenu }) {
    ...
    const riskyPresentation = !token.isNative && usesRiskPresentation(token)
    const riskLabel = riskyPresentation
        ? primaryReasonLabel(token) ?? (token.visibility === 'unverified'
            ? 'Unverified'
            : 'Potential risk')
        : null
