...
const NATIVE_TOKEN_KEY_SUFFIX = ':0x0000000000000000000000000000000000000000'
...
function shouldHideCatalogWrappedNativeToken(token, { exactAddressSearch, walletTokensByKey }) {
    if (exactAddressSearch) return false
    if (!isWrappedNativeToken(token)) return false
    return !walletTokensByKey.has(getTokenKey(token))
}
...
export function useTokenSelectorState({
    ...
    const saveRecentToken = useCallback((token) => {
        if (!normalizedSearch || !isTrustedWalletToken(token)) return
