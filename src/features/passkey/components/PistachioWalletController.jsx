...
import { KeyRound, Lock, WalletCards, X } from 'lucide-react'
...
export default function PistachioWalletController() {
    ...
    async function resumeRestoredVault() {
        setEntryScreen('menu')
        try {
            if (
                snapshot.signingPasskeyOnly === true &&
            ...
            // The restored ciphertext remains saved, and the manager publishes a safe error.
        }
    }
            ...
            <Dialog.Root open={open}>
                <Dialog.Portal>
                    ...
                    >
                        ...
                        <p className="pistachio-sr-only" id="pistachio-wallet-dialog-description">Create, import, restore, and manage Pistachio Wallet. Passkey confirmation protects signing and sensitive security actions.</p>
    ...
    return (
        <button type="button" className="header-icon-button" aria-label="Open Pistachio Wallet" title="Pistachio Wallet" onClick={() => {
            ...
            {snapshot.signingPasskeyOnly === true
                ...
                : snapshot.phase === 'unlocked' || snapshot.resumeReauthPending
