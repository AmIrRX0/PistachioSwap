...
describe.sequential('quote normalization and selection', () => {
    ...
    it('calculates minimum-chargeable and uncapped Uniswap fees with integer arithmetic', () => {
        expect(calculateUniswapEffectiveFee({ grossOutputRaw: '41', configuredBps: 67, maximumBps: 500 })).toMatchObject({ effectiveBps: 244, feeAmountRaw: '1', adjustment: 'minimum-chargeable', capped: false })
        expect(calculateUniswapEffectiveFee({ grossOutputRaw: '100', configuredBps: 67, maximumBps: 500 })).toMatchObject({ effectiveBps: 100, feeAmountRaw: '1', capped: false })
        expect(calculateUniswapEffectiveFee({ grossOutputRaw: '150', configuredBps: 67, maximumBps: 500 })).toMatchObject({ effectiveBps: 67, feeAmountRaw: '1', capped: false })
        expect(() => calculateUniswapEffectiveFee({ grossOutputRaw: '19', configuredBps: 67, maximumBps: 500 })).toThrowError(expect.objectContaining({ code: 'UNISWAP_FEE_ROUTE_TOO_SMALL' }))
        expect(calculateUniswapEffectiveFee({ grossOutputRaw: '50000', configuredBps: 67, maximumBps: 500 })).toMatchObject({ effectiveBps: 67, feeAmountRaw: '335', adjustment: 'base', capped: false })
    })
