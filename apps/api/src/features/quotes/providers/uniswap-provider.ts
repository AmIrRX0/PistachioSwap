...
export function calculateUniswapEffectiveFee({
    ...
    if (gross <= 0n) {
        throw new ProviderError({ code: 'UNISWAP_FEE_OUTPUT_INVALID', message: 'The gross output amount is invalid.', outcome: 'validation' })
    }
    ...
    const effectiveBps = Math.max(configuredBps, Number(minimumChargeableBps))
    if (effectiveBps > maximumBps) {
        throw new ProviderError({ code: 'UNISWAP_FEE_TOO_HIGH', message: 'The platform fee exceeds the provider maximum.', outcome: 'configuration' })
    }
    const feeAmountRaw = (gross * BigInt(effectiveBps)) / BPS_DENOMINATOR
    if (feeAmountRaw < 1n) {
        throw new ProviderError({ code: 'UNISWAP_FEE_NOT_CHARGEABLE', message: 'The platform fee cannot be represented safely for this route.', outcome: 'validation' })
    }
    return {
        effectiveBps,
        feeAmountRaw: feeAmountRaw.toString(),
        adjustment: effectiveBps > configuredBps ? 'minimum-chargeable' as const : 'base' as const,
        capped: false,
    }
}
...
export function createUniswapProvider({ applyPlatformFee = true }: { applyPlatformFee?: boolean } = {}): QuoteProvider {
    ...
    return {
        ...
        async getQuote(request, signal) {
            ...
            let feeAdjustment: 'base' | 'minimum-chargeable' | null = null
            ...
            if (integratorFee && effectiveIntegratorFee && grossBuyAmount) {
