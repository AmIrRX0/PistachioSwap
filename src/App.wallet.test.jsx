...
function findTokenRowByText(text) {
    const dialogs = [...document.querySelectorAll('.ps-token-selector-dialog')]
    const root = dialogs.at(-1) ?? document
    return [...root.querySelectorAll('.ps-token-row')]
        .find((row) => row.textContent.includes(text))
}

function findNativeBnbRow() {
    return findTokenRowByText('BNB') ??
        [...((document.querySelectorAll('.ps-token-selector-dialog').item(document.querySelectorAll('.ps-token-selector-dialog').length - 1)) ?? document).querySelectorAll('.ps-token-row')]
            .find((row) => row.textContent.includes('Native token'))
}
