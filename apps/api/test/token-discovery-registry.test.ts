...
function token(chainId: number, volume24hUsd: number): MarketToken {
    const address = `0x${String(chainId).padStart(40, '0')}`
    const logoURI = `https://example.com/${chainId}.png`
    const oldestPairCreatedAt = '2020-01-01T00:00:00.000Z'
    return {
        id: `${chainId}:${address}`,
        canonicalId: `${chainId}:${address}`,
        chainId,
        address,
        name: `Token ${chainId}`,
        symbol: `T${chainId}`,
        ...
        trustedLiquidityUsd: 1_000_000,
        largestTrustedPoolLiquidityUsd: 1_000_000,
        transactions24h: 100,
        transactionCount24h: 100,
        pairCount: 1,
        trustedPairCount: 1,
        oldestPairCreatedAt,
        oldestTrustedPoolCreatedAt: oldestPairCreatedAt,
        marketUrl: null,
        rank: 1,
