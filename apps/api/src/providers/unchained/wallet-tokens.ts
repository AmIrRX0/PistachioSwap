...
export type UnchainedWalletTokenResult = {
    ...
    chainDiagnostics: Record<string, ChainFailure>
    batchErrors: []
    partial: boolean
    stale: false
    diagnostics: {
...
export function normalizeUnchainedAccount(value: unknown): Account | null {
    const candidate = isRecord(value) && isRecord(value.account)
        ? value.account
        : value
    if (!isRecord(candidate)) return null
    const balance = nonnegativeInteger(candidate.balance)
    const pubkey = normalizeAddress(candidate.pubkey)
    if (balance === null || !pubkey || !Array.isArray(candidate.tokens)) return null
    const tokens: AccountToken[] = []
    for (const item of candidate.tokens) {
        if (!isRecord(item)) continue
        const contract = normalizeAddress(item.contract)
        const tokenBalance = nonnegativeInteger(item.balance)
        const decimals = validDecimals(item.decimals)
        const name = cleanText(item.name, 120)
        const symbol = cleanText(item.symbol, 32)
...
function endpointHostPort(endpoint: string) {
...
function chainFailure(
    ...
    return {
        ...
        code,
        field,
        endpoint: endpointHostPort(endpoint),
    } satisfies ChainFailure
}

function classifyFetchFailure(endpoint: string, error: unknown): ChainFailure {
    const nestedCause = error instanceof Error ? error.cause : null
    const nested = nestedCause instanceof Error ? nestedCause : null
    const nestedCode = isRecord(nestedCause) && typeof nestedCause.code === 'string'
        ? nestedCause.code
        : nested?.cause && isRecord(nested.cause) && typeof nested.cause.code === 'string'
          ? nested.cause.code
          : null
    ...
    if (error instanceof ProviderError) {
        if (error.code === 'PROVIDER_RESPONSE_INVALID') {
            return chainFailure(endpoint, 'invalid-json', 'UNCHAINED_INVALID_JSON')
        }
        if (nestedCode === 'ECONNREFUSED') {
            return chainFailure(endpoint, 'connection-refused', 'UNCHAINED_CONNECTION_REFUSED')
        }
        if (nestedCode === 'ENOTFOUND' || nestedCode === 'EAI_AGAIN') {
            return chainFailure(endpoint, 'dns', 'UNCHAINED_DNS_FAILURE')
        }
        if (error.outcome === 'timeout' || nestedName === 'TimeoutError' || /timeout|timed out/i.test(nestedMessage)) {
            return chainFailure(endpoint, 'timeout', 'UNCHAINED_TIMEOUT')
        }
        if (error.upstreamStatus !== null) {
            return chainFailure(endpoint, 'http-status', `UNCHAINED_HTTP_${error.upstreamStatus}`)
        }
        return chainFailure(endpoint, 'http-status', error.code)
    }
    const cause = error instanceof Error ? error.cause : null
    const code = isRecord(cause) && typeof cause.code === 'string'
        ? cause.code
    ...
    return chainFailure(endpoint, 'http-status', 'UNCHAINED_REQUEST_FAILED')
}

function circuitOpen(chainId: number) {
    const entry = circuits.get(chainId)
    return entry && entry.openedUntil > Date.now() ? entry : null
}

function recordCircuitFailure(chainId: number, failure: ChainFailure) {
    if (!['connection-refused', 'timeout', 'dns'].includes(failure.category)) return
    const current = circuits.get(chainId)
    const failures = (current?.failures ?? 0) + 1
    const cooldownMs = failures >= CIRCUIT_FAILURE_THRESHOLD
        ? Math.min(
              current?.cooldownMs
                  ? current.cooldownMs * 2
                  : CIRCUIT_INITIAL_COOLDOWN_MS,
              CIRCUIT_MAX_COOLDOWN_MS,
          )
        : 0
    circuits.set(chainId, {
        failures,
        cooldownMs,
        openedUntil: cooldownMs ? Date.now() + cooldownMs : 0,
    })
}

export function closeUnchainedCircuitForChain(chainId: number) {
    circuits.delete(chainId)
}
...
async function fetchAccount(
    ...
    let payload: unknown
    try {
        payload = await fetchJson(accountUrl(endpoint, walletAddress), {
            signal,
        ...
        throw classifyFetchFailure(endpoint, error)
    }
    ...
    if (!account) {
        throw chainFailure(
            endpoint,
            'schema-mismatch',
            'UNCHAINED_ACCOUNT_SCHEMA_MISMATCH',
            'account',
        )
    }
    if (account.pubkey !== walletAddress) {
        throw chainFailure(
            endpoint,
            'wallet-address-mismatch',
            'UNCHAINED_WALLET_ADDRESS_MISMATCH',
            'pubkey',
        )
    }
    return account
...
async function loadChainTokens({
    ...
    if (open) {
        throw chainFailure(endpoint, 'circuit-open', 'UNCHAINED_CIRCUIT_OPEN')
    }
    ...
    if (!request) {
        request = (async () => {
            ...
            closeUnchainedCircuitForChain(chainId)
            return tokens
...
export async function getUnchainedWalletTokens({
    ...
    const chainDiagnostics: Record<string, ChainFailure> = {}
    let pageCount = 0
    ...
    const workers = Array.from({ length: Math.min(4, requested.length) }, async () => {
        while (cursor < requested.length) {
            ...
            try {
                ...
                const endpoint = endpoints.get(chainId)!
                const failure = isRecord(error) && typeof error.category === 'string'
                    ? error as ChainFailure
                    : chainFailure(endpoint, 'http-status', 'UNCHAINED_REQUEST_FAILED')
                recordCircuitFailure(chainId, failure)
                failedChainIds.push(chainId)
                chainDiagnostics[String(chainId)] = failure
                chainErrors[String(chainId)] =
                    `${failure.code}:${failure.category}${failure.field ? `:${failure.field}` : ''}`
            }
        }
    })
    ...
    if (successfulChainIds.length === 0) {
        throw new ProviderError({
            ...
            retryable: true,
            outcome: 'upstream',
            providers: Object.entries(chainDiagnostics).map(([chainId, failure]) => ({
                provider: 'unchained',
                outcome: failure.category === 'missing-configuration'
                    ? 'configuration'
                    : failure.category === 'timeout'
                      ? 'timeout'
                      : 'upstream',
                category: failure.category === 'timeout'
                    ? 'timeout'
                    : failure.category === 'missing-configuration'
                      ? 'configuration-error'
                      : 'temporary-failure',
                upstreamStatus: null,
                code: `${chainId}:${failure.code}`,
                message: `Unchained chain ${chainId} failed with ${failure.category}.`,
                retryable: true,
            })),
        })
    }
