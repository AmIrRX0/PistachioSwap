...
describe('Pistachio Wallet entry and modal behavior', () => {
    ...
    it('renders one accessible modal through document.body with required no-vault wording', () => {
        ...
        expect(screen.getByText('Create a wallet or securely import one you already own.')).toBeTruthy()
    ...
    it('reopens from a clean no-vault menu after close', async () => {
        ...
        expect(await screen.findByText('Create a wallet or securely import one you already own.')).toBeTruthy()
    })
...
describe('Pistachio Wallet saved-wallet behavior', () => {
    ...
    it('shows previous-wallet detection, shortened address, and no chooser for one vault', async () => {
        ...
        expect(screen.getByText('Unlock your saved wallet or use a different one.')).toBeTruthy()
    ...
    it('reveals a fake recovery value only after reauthentication and supports manual lock', async () => {
        ...
        expect(within(recoveryRegion).getByText('test-only')).toBeTruthy()
        expect(within(recoveryRegion).getByText('recovery')).toBeTruthy()
        expect(within(recoveryRegion).getByText('phrase')).toBeTruthy()
        await user.click(within(recoveryRegion).getByRole('button', { name: 'Hide' }))
        expect(screen.queryByRole('region', { name: 'Recovery phrase' })).toBeNull()
