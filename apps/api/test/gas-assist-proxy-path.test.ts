...
/*
 * `app.inject()` normalizes the request target before routing, which hides
 * every path-confusion bug in this proxy: the same inputs that reach the
 * handler over a real socket are rewritten to a 404 under inject. These tests
 * therefore speak HTTP/1.1 down an actual TCP connection.
 */
...
describe('Gas Assist proxy path confinement', () => {
    ...
    it.each([
        ...
        try {
            ...
            expect(response).toContain('GAS_ASSIST_ROUTE_NOT_FOUND')
